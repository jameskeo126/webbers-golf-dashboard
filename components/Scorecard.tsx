          ...
          <thead>
            <tr>
              ...
              {Array.from({ length: 9 }, (_, i) => (
                <th key={i} className="h-10 w-20 sm:w-16 text-center font-medium" style={headerCellStyle}>{i + 1}</th>
              ))}
              <th className="h-10 w-20 text-center font-semibold" style={headerCellStyle}>OUT</th>
              {Array.from({ length: 9 }, (_, i) => (
                <th key={i + 9} className="h-10 w-20 sm:w-16 text-center font-medium" style={headerCellStyle}>{i + 10}</th>
              ))}
              <th className="h-10 w-20 text-center font-semibold" style={headerCellStyle}>IN</th>
              <th className="h-10 w-20 text-center font-semibold" style={headerCellStyle}>TOT</th>
            </tr>
            <tr>
              ...
              {HOLE_PARS.slice(0, 9).map((p, i) => (
                <td key={i} className="h-10 w-20 sm:w-16 text-center" style={headerCellStyle}>{p}</td>
              ))}
              <td className="h-10 w-20 text-center font-semibold" style={headerCellStyle}>{FRONT_9_PAR}</td>
              {HOLE_PARS.slice(9).map((p, i) => (
                <td key={i + 9} className="h-10 w-20 sm:w-16 text-center" style={headerCellStyle}>{p}</td>
              ))}
              <td className="h-10 w-20 text-center font-semibold" style={headerCellStyle}>{BACK_9_PAR}</td>
              <td className="h-10 w-20 text-center font-semibold" style={headerCellStyle}>{FRONT_9_PAR + BACK_9_PAR}</td>
            </tr>
          </thead>
          <tbody>
            {PLAYERS.map(player => {
              ...
              return (
                <tr key={player.id}>
                  <td className="sticky left-0 pl-1 pr-3 py-1 text-left whitespace-nowrap text-lg font-semibold" style={{ backgroundColor: "var(--bg-surface)", border: "1px solid var(--border)", color: "var(--text-primary)" }}>
                  ...
                  <td className="h-10 w-20 text-center font-semibold tabular" style={totalCellStyle}>
                    {out === null ? "—" : out}
                  </td>
